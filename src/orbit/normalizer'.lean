...
lemma normalizer_neq_subgroup [fintype G] (H : subgroup G) {p : ℕ} 
  (hp: p.prime) (h: is_p_subgroup H p) : 
  p ∣ (index H) → normalizer (H : set G) ≠ H := sorry

theorem sylow_one_part1 [fintype G] {p m n: ℕ} {hp : p.prime} 
